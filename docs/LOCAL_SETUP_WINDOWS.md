...
## Scheduler and communication checks

Prompt 22 registers hourly due-reminder processing and a daily vendor-contract expiry check. Both commands are idempotent and may be run manually when local cron is unavailable:

~~~powershell
php artisan notifications:send-due-reminders
php artisan notifications:check-expiries --days=30
php artisan schedule:list
php artisan schedule:run
~~~

On cPanel, configure one cron entry to run every minute using the hosting account's absolute PHP and project paths:

~~~text
* * * * * /usr/local/bin/php /home/ACCOUNT/EventManagement/artisan schedule:run >> /dev/null 2>&1
~~~

Replace both paths with values confirmed by the host. Production email remains disabled until `features.communications_enabled` is enabled in Organization Settings and a valid non-placeholder Laravel mail transport/sender is configured. Local/testing `log` or `array` mail is intentionally available for verification. SMS and WhatsApp adapters remain disabled and retain failed attempt history until real providers are implemented; no queue worker, Redis, WebSocket, or browser-push service is needed.
